      ...
      {mission.champs && mission.champs.length > 0 ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          {mission.champs.map((champ) => (
            <ChampCard key={champ.cle} champ={champ} reponses={reponses} setChamp={setChamp} />
          ))}
        ...
        <PremiumCard title="Rédigez avec l&apos;IA" className="mt-6">
          <div className="space-y-4">
            {mission.prompts_ia.map((p, i) => {
              const resolvedPrompt = replacePromptVariables(p.prompt, reponses, promptVariableMap);
              return (
        ...
        isElite ? (
          ...
          <PremiumCard className="mt-6 border-l-[3px] border-l-gold" glow>
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="font-semibold text-dark">Contenu Elite</p>
            </div>
            <p className="text-sm text-secondary">
              Les abonnés Elite reçoivent un tunnel de vente premium prêt à importer dans Systeme.io.
            </p>
            <Link
              href="/abonnement"
              className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-ochre hover:underline"
            >
...
function ChampCard({
  ...
  setChamp: (cle: string, val: string | string[]) => void;
}) {
  const baseClass =
  ...
  return (
    <PremiumCard title={champ.libelle + (champ.obligatoire ? " *" : "")}>
      {champ.aide && (
        <p className="mb-2 text-xs text-secondary">{champ.aide}</p>
      )}
      <ChampInput champ={champ} value={reponses[champ.cle] ?? ""} onChange={(val) => setChamp(champ.cle, val)} />
    </PremiumCard>
  );
}
